...
class WindowFunctionExecutor : public AbstractExecutor {
 public:
  ...
  auto MakeWinKey(const Tuple *tuple, const std::vector<AbstractExpressionRef> &partition_by_exprs) -> AggregateKey {
    std::vector<Value> keys;
    keys.reserve(partition_by_exprs.size());

    for (const auto &expr : partition_by_exprs) {
      keys.push_back(expr->Evaluate(tuple, this->GetOutputSchema()));
    }

    return AggregateKey{std::move(keys)};
  }
  auto MakeWinValue(const Tuple *tuple, const AbstractExpressionRef &window_expr) {
    // Evaluate the window expression against the tuple and return the resulting value
    return window_expr->Evaluate(tuple, this->GetOutputSchema());
  }
